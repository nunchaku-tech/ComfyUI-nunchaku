...
def main(precision: str):
    ...
    with torch.inference_mode():
        ...
        cliptextencode_6 = cliptextencode.encode(
            text="holding an sign saying 'SVDQuant is fast!'",
            clip=get_value_at_index(nunchakutextencoderloaderv2_51, 0),
        )
        ...
        loadimage_50 = loadimage.load_image(image="monalisa.jpg")

        nunchakuipadapterloader = NODE_CLASS_MAPPINGS["NunchakuIPAdapterLoader"]()
        nunchakufluxipadapterapply = NODE_CLASS_MAPPINGS["NunchakuFluxIPAdapterApply"]()
        ...
        for q in range(1):
            nunchakuipadapterloader_48 = nunchakuipadapterloader.load(
                model=get_value_at_index(nunchakufluxditloader_45, 0)
            )

            nunchakufluxipadapterapply_49 = nunchakufluxipadapterapply.apply_ipa(
                weight=1.1500000000000001,
                model=get_value_at_index(nunchakuipadapterloader_48, 0),
                ipadapter_pipeline=get_value_at_index(nunchakuipadapterloader_48, 1),
