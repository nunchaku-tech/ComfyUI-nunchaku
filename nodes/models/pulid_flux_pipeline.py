...
from nunchaku.models.pulid.utils import img2tensor, tensor2img#, resize_numpy_image_long

import folder_paths


class PuLIDPipeline(nn.Module):
    def __init__(self, dit, device, weight_dtype=torch.bfloat16, onnx_provider="gpu", clip_vision_path:str=None, *args, **kwargs):
        super().__init__()
        self.device = device
        ...
        facedetection_model_root = os.path.join(folder_paths.models_dir, "facedetection") if os.path.join(folder_paths.models_dir, "facedetection/detection_Resnet50_Final.pth") and os.path.join(folder_paths.models_dir, "facedetection/parsing_bisenet.pth") else None #Use existing models because some other custom-node need it too.
        self.face_helper = FaceRestoreHelper(
            ...
            save_ext="png",
            device=self.device,
            model_rootpath=facedetection_model_root
        )
        self.face_helper.face_parse = None
        self.face_helper.face_parse = init_parsing_model(model_name="bisenet", device=self.device)
        # clip-vit backbone
        if clip_vision_path == None:
            hf_hub_download(
                "QuanSun/EVA-CLIP",
                "EVA02_CLIP_L_336_psz14_s6B.pt",
                local_dir=os.path.join(folder_paths.models_dir, "clip_vision")
            )
            clip_vision_path = os.path.join(folder_paths.models_dir, "clip_vision/EVA02_CLIP_L_336_psz14_s6B.pt")
        model, _, _ = create_model_and_transforms(
            "EVA02-CLIP-L-14-336",
            pretrained=clip_vision_path,
        ...
        # antelopev2
        if os.path.exists(os.path.join(folder_paths.models_dir, "antelopev2/glintr100.onnx")):
            insightface_root = folder_paths.models_dir
            glintr100_path = os.path.join(folder_paths.models_dir, "antelopev2/glintr100.onnx")
        else:
            insightface_root = os.path.join(folder_paths.models_dir, "insightface")
            glintr100_path = os.path.join(folder_paths.models_dir, "insightface/models/antelopev2/glintr100.onnx")
            if not os.path.exists(os.path.join(folder_paths.models_dir, "insightface/models/antelopev2/glintr100.onnx")):
                snapshot_download("DIAMONIK7777/antelopev2", local_dir=os.path.dirname(glintr100_path))
        ...
        self.handler_ante = insightface.model_zoo.get_model(glintr100_path, providers=providers)
    ...
    def load_pretrain(self, pretrain_path=None, version="v0.9.0"):
        ...
        else: # Modify download path into ipadapter
            hf_hub_download("guozinan/PuLID", f"pulid_flux_{version}.safetensors", local_dir=os.path.join(folder_paths.models_dir, "ipadapter"))
            ckpt_path = os.path.join(folder_paths.models_dir, f"ipadapter/pulid_flux_{version}.safetensors")
