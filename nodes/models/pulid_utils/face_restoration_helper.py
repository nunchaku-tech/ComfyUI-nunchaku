"""
Borrowed from https://github.com/lldacing/ComfyUI_PuLID_Flux_ll/blob/main/face_restoration_helper.py
"""
...
def get_face_by_index(det_faces, face_sort_rule, face_index=0):
    ...
    if face_sort_rule == "left-right":
        ...
        sorted_faces = sorted(
            indexed_faces,
            key=lambda x: (
                (x[1].bbox[2] - x[1].bbox[0]) * (x[1].bbox[3] - x[1].bbox[1])
                if has_bbox_attr
                else (x[1][2] - x[1][0]) * (x[1][3] - x[1][1])
            ),
        )
...
class FaceRestoreHelper(object):
    ...
    def __init__(
        ...
        self.face_parse = init_parsing_model(
            model_name=parsing_model, device=self.device, model_rootpath=model_rootpath
        )
    ...
    def get_face_landmarks_5(
        ...
        if ref_sort_bboxes is not None:
            ...
            self.det_faces, center_idx, _ = get_face_by_index(
                self.det_faces, face_sort_rule=face_sort_rule, face_index=face_index
            )
