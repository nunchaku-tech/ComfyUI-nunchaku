...
try:
    from .radial_attn_core import RadialAttention, MaskMap
except ImportError:
...
def get_radial_attn_func(video_token_num, num_frame, block_size, decay_factor, ra_options):
    ...
    def radial_attn_func(q, k, v, heads, mask=None, attn_precision=None, skip_reshape=False, skip_output_reshape=False):
        ...
        out = RadialAttention(q, k, v, mask_map=mask_map, sparsity_type=sparsity_type, 
                            block_size=block_size, decay_factor=decay_factor, model_type="wan")
...
class PatchRadialAttn:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "model": ("MODEL",),
                "dense_layers": ("STRING", {"default": "1,2", "tooltip": "Comma-separated layer indices to apply radial attention (e.g., '1,2,3')"}),
                "dense_timesteps": ("INT", {"default": 2, "min": 0, "max": 100, "step": 1, "tooltip": "Apply radial attn from which timestep."}),
    ...
    def patch_radial_attn(self, model, dense_layers, dense_timesteps, block_size, decay_factor):
        ...
        try:
            ...
            raise ValueError(f"Invalid dense_layers format: {dense_layers}. Expected comma-separated integers like '1,2,3'")
        ...
        print(f"✅ RADIAL ATTENTION PATCHED:")
...
class RadialAttentionInfo:
    ...
    def get_info(self, video_length, width, height, num_frames, block_size, decay_factor, dense_layers, dense_timesteps):
        """
